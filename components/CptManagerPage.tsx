...
import { WordPressCredentials, WordPressPost } from '../types';
import { deleteCPTItem, getCPTItems } from '../services/wordpressService';
import { DocumentTextIcon, PencilIcon, TrashIcon, ImageIcon } from './icons/Icons';

interface CptManagerPageProps {
  postType: string;
  postTypeName: string;
...
const CptManagerPage: React.FC<CptManagerPageProps> = ({ postType, postTypeName, wordPressCredentials, onCreate, onEdit }) => {
  ...
  const fetchItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const { posts, totalPages: fetchedTotalPages } = await getCPTItems(wordPressCredentials, postType, currentPage, 10, selectedStatus);
      setItems(posts);
  ...
  }, [wordPressCredentials, postType, postTypeName, currentPage, selectedStatus]);
                                ...
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap max-w-sm truncate">
                                        <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-brand-blue-dark hover:text-brand-cyan" title={item.title.rendered}>
                                            {item.title.rendered ? decodeHtmlEntities(item.title.rendered) : 'Sem título'}
                                        </a>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
